...
class InsuredDriversBottomSheet extends StatelessWidget {
  const InsuredDriversBottomSheet({
    ...
    return BlocBuilder<SubmitClaimBloc, SubmitClaimState>(
      builder: (context, state) {
        ...
        if (state is SubmitClaimFormInitSuccess) {
          for (final element in state.claimFormData.insuredDrivers) {
            if (element != null) {
              data.add(
          ...
          data.add(
            BottomSheetSelectorOption(
              label: context.getLocalizationOf.insuredDriverOtherLabel,
              value: context.getLocalizationOf.insuredDriverOtherLabel,
            ),
          );
        }
        ...
        return Semantics(
          ...
          child: BottomSheetSelector(
            title: context.getLocalizationOf.insuredDriverFieldLabel,
            options: data,
            onChanged: onChanged,
            requiredField: true,
